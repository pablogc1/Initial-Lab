# -*- coding: utf-8 -*-
"""
Created on Fri Aug 30 19:50:14 2024
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if line.startswith("Iteration"):
                ...
                num_elements_info = line.split(",")[2].strip()
                current_entry = {
                    'description': f"{iteration_info}, {num_sets_info}, {num_elements_info}, {line.split('Running with')[1].strip()}",
...
def plot_data_based_on_tags(data, selected_tags, num_sets=None, num_elements=None, iteration=None, plot_all_num_sets=False, plot_all_iterations=False, plot_all_num_elements=False):
    plots_made = False
    for entry in data:
        if tags_match(selected_tags, entry['description']):
            ...
            if plot_all_iterations or (iteration and entry['iteration'] == iteration):
                ...
                if entry['type'] == 'all_vs_all':
                    ...
                    label = f"Iteration {entry['iteration']}, num_elements={entry['num_elements']}" if plot_all_num_elements else f"Iteration {entry['iteration']}"
...
def main():
    ...
    selected_tags = ['regular_uniqueness', 'strong', 'fixed', 'one_vs_all', 'continuous=False']  # Example tags
    ...
    plot_data_based_on_tags(data, selected_tags, num_sets=num_sets, num_elements=num_elements, iteration=iteration, plot_all_num_sets=plot_all_num_sets, plot_all_iterations=plot_all_iterations, plot_all_num_elements=plot_all_num_elements)
