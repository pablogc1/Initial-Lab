# -*- coding: utf-8 -*-
"""
Created on Tue Aug 27 13:30:17 2024
...
def generate_sets(start_num, iterations):
    sets_dict = {}
    to_process = [(start_num, start_num + 1, 1 / (start_num + 1))]  # Initialize with the first set
    seen = set()
    ...
    for _ in range(iterations):
        ...
        if s_plus_1 not in seen:
            to_process.append((s_plus_1, s_plus_1 + 1, 1 / (s_plus_1 + 1)))
            seen.add(s_plus_1)
        
        if f_s_plus_1 not in seen:
            to_process.append((f_s_plus_1, f_s_plus_1 + 1, 1 / (f_s_plus_1 + 1)))
            seen.add(f_s_plus_1)
...
def expand_undeveloped_elements(undeveloped_elements, sets_dict, iterations=5):
    for el in undeveloped_elements:
        s_plus_1 = el + 1
        f_s_plus_1 = 1 / (el + 1)
        sets_dict[el] = [s_plus_1, f_s_plus_1]
        # Develop further if necessary
        to_process = [(s_plus_1, s_plus_1 + 1, 1 / (s_plus_1 + 1)),
                      (f_s_plus_1, f_s_plus_1 + 1, 1 / (f_s_plus_1 + 1))]
        for _ in range(iterations):
            if not to_process:
                break
            s, s_plus_1, f_s_plus_1 = to_process.pop(0)
...
def select_and_compare(sets_dict, selected_set, iterations=5):
    ...
    for set_num_b in list(sets_dict.keys()):  # Compare with all other sets
        if selected_set != set_num_b:
            ...
            if undeveloped_elements:
                ...
                expand_undeveloped_elements(undeveloped_elements, sets_dict, iterations)
...
# Generate sets using f(x) = 1/x and starting with s=1
sets_dict = generate_sets(start_num=1, iterations=1000)
...
select_and_compare(sets_dict, selected_set=1)
